...
class Train(object):
    def __init__(self, config):
        ...
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[config["step_size"]], gamma=config["gamma"])
    ...
    def train_one_epoch(self):
        ...
        for batch_images, batch_labels in self.dataloader.get_train_batch(is_aug = config["augment"]):
            ...
            # loss = dummy_loss(pred, batch_labels)
    ...
    def evaluate_on_train(self):
        train_images = [label["resized_image"] for label in self.dataloader.train_labels]
        train_boxes = [label["resized_boxes"] for label in self.dataloader.train_labels]
